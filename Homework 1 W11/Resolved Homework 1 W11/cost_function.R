...
set.seed(123)
sample  =  sample.split(heuristic_df,SplitRatio = 0.75)
h_train =  subset(heuristic_df,sample ==TRUE)
h_test  =  subset(heuristic_df, sample==FALSE)
...
True_p_true = length(which(h_test$True.Positive==TRUE))
True_n_true = length(which(h_test$True.Negative==TRUE))
num_of_guesses = length(h_test$Guess)

Calculation = (True_p_true + True_n_true)/(num_of_guesses)
...
Num_true_p  = length(h_test$True.Positive)
Num_false_p = length(h_test$False.Positive)

cla2 = (Num_true_p)/(Num_true_p + Num_false_p)
...
Num_false_n = length(h_test$False.Negative)

cla3 = (Num_true_p)/(Num_true_p + Num_false_n)
